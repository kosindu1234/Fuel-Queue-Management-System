...
public class queue {
    public static String[] waitingList = new String[100];
    public static String[] Snemelist = new String[100];
    public static int[] Vnumberlist = new int[100];
    public static int[] literslist = new int[100];
    public static void resetQueue(){
        ...
        for (int i=0;i < waitingList.length; i++){
            waitingList[i]=waitingList[i++];
            Snemelist[i]=Snemelist[i++];
            Vnumberlist[i]=Vnumberlist[i++];
            literslist[i]=literslist[i++];
        }
    }
    ...
    public static FuelQueue [] add(FuelQueue pump[] ,int n,int a) {
        Scanner input = new Scanner(System.in);
        int s =0;
        if (a>=6){
            System.out.println("all pumps are full..\n Add customer to queue");
            System.out.println("\nAdd passenger Frist Name");
            waitingList[s] = input.next();
            System.out.println("Add passenger second Name");
            Snemelist[s] = input.next();
            System.out.println("Add vehicle number");
            Vnumberlist[s] = input.nextInt();
            System.out.println("Add required no of liters");
            literslist[s] = input.nextInt();
            s++;
        }
    ...
    public static FuelQueue [] delete(FuelQueue delete[] , String name){
        for(int c = 0; c<6; c++){
            if(delete[c].firstname[c].equals(name)){
                delete[c].firstname[c]=waitingList[0];
                delete[c].secondname[c]=Snemelist[0];
                delete[c].vehiclenumber[c]=Vnumberlist[0];
                delete[c].noliter[c]=literslist[0];
                resetQueue();
            }
        }
        return delete;
    ...
    public static FuelQueue [] delete2(FuelQueue delete[] ){
        delete[0].firstname[0]=waitingList[0];
        delete[0].secondname[0]=Snemelist[0];
        delete[0].vehiclenumber[0]=Vnumberlist[0];
        delete[0].noliter[0]=literslist[0];
        resetQueue();
        return delete;
    }
    ...
    public static FuelQueue[] sort(FuelQueue[] pump_1,FuelQueue[] pump_2,FuelQueue[] pump_3,FuelQueue[] pump_4 ,FuelQueue[] pump_5){    //https://www.softwaretestinghelp.com/sort-arrays-in-java/
        ...
        try{
            ...
            if (queue_no==1){
                ...
                for (int g = 0; g < n; g++) {
                    System.out.println(pump_1[g].firstname[g]);
                }
                System.out.println(" ");
            } else if (queue_no==2) {
                int n=6;
